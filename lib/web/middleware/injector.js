...
//      return date_object.toLocaleString();
  ...
  navbar: function(){
    ...
    if (req.project !== undefined) {
      ...
      github_config = project.get_api_config('github');
      v1_config = project.get_api_config('version_one');

      if (v1_config) {
        _navbar['VersionOne'] = urls.DEFECTS.replace(':project_name', name);
      }
