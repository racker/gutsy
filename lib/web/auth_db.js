...
var AuthDB = utils.make_class({
  ...
  _validateSHA: function(password, hash) {
    return sha1crypt(password) === hash;
  },

  _validateMD5: function(password, hash) {
    var all = hash.split('$');
    return md5crypt(password, all[2], '$apr1$') === hash;
  }
});

exports.install = function(filePath) {

  var dir = fs.readdirSync(settings.idjson_path);
