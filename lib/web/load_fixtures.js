 ...
 */
module.exports = function load_devops(devops, cb) {
  var extcreds;
  devops = devops || {};
  ...
  db.get_devops(undefined, function (err, results) {
    _.each(results, function(row){
      var api_name;
      var api_obj;
      var devops_json = row.data ? JSON.parse(row.data): {};
      var project_name = row.project;
      ...
      devops_json.project = project_name;
      devops_json.url = row.url;
      devops_json.id = row.id;
      devops_json.updated_at = row.updated_at;

      devops[project_name] = devops_json;
    });
