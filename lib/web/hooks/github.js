...
var get_hooks = function(callback){
  ...
  var list_hooks = {};
  var options;
  options = new github_request(self.devops.repo, self.devops.github, "GET");
  ...
  _.each(JSON.parse(result.data), function(hook){
    ...
    if (hook.config.url === self.get_api_url(repo)){
      ...
      if (!delete_hook){
        ...
        options = new github_request(repo, self.devops, "DELETE", hook.id);
...
var add_hook = function(results, existing_hooks, callback){
  ...
  options = github_request(self.project_name, self.devops, "POST");
