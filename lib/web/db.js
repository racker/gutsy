...
module.exports = new utils.make_class({
  ...
  update_project: function (id, name, url, devops_json, creds, cb) {
    ...
    self.get_project(id, function(err, old_project){
      var old_json = JSON.parse(old_project.devops_json);
      var update = false;

      if (old_project.name !== name || old_project.url !== url) {
        update = true;
      } else if (creds !== undefined && !_.isEqual(JSON.parse(creds), JSON.parse(old_project.creds))) {
        update = true;
      } else if (devops_json !== undefined && !_.isEqual(parsed, old_json)) {
        update = true;
      }

      if (update === false){
        old_project.updated = false;
        return cb(err, old_project);
      }

      console.log(parsed, devops_json, id, name, url, creds);
      return self.db.run(query_str, query_args, function(err){
        if (err){
          log.error(err);
          return cb(err);
        }
        self.get_project(id, function(err, project){
          project.updated = true;
          project.old_name = old_project.name;
          cb(err, project);
        });
      });
    });
