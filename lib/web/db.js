...
module.exports = new utils.make_class({
  ...
  add_event: function(event, project, cb){
    ...
    if (!event.user){
      log.warn("No user for event. Not inserting.", event);
  ...
  del_service_stat: function(project, hostname, cb) {
    var self = this;
    self.db.run("DELETE FROM metrics WHERE project = ? AND service_name = ?;",
      [project, hostname], self.squelch_integrity_error(cb));
  },
