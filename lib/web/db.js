...
var SCHEMA = ["CREATE TABLE IF NOT EXISTS events (project VARCHAR(40) NOT NULL, " +
  ...
  "alias TEXT NOT NULL, related_api VARCHAR(40) NOT NULL, UNIQUE(alias, related_api), UNIQUE(alias, alias_display_id));"
];
...
module.exports = new utils.make_class({
  ...
  add_alias: function(alias, display_name, related_api, cb){
    ...
    self.db.run("INSERT INTO alias_display (display_name) VALUES (?);",
      [display_name],
      function () {
        self.db.run("INSERT INTO aliases(alias, related_api, alias_display_id) VALUES(?, ?, (SELECT id FROM alias_display WHERE alias_display.display_name=?));",
        [alias, related_api, display_name], cb);
      }
    );
  },
  ...
  delete_alias: function(display_name, cb){
    var self = this;
    self.db.run("DELETE FROM aliases WHERE alias_display_id =(SELECT id FROM alias_display WHERE display_name=?);",
      display_name, cb
    );
  },
