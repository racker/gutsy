...
module.exports = new utils.make_class({
  ...
  add_service_stat: function(project, host_data, cb){
    self.db.run("INSERT OR REPLACE INTO metrics (id, project, service_name, health, ip, time, services) " +
      "VALUES ((select id from metrics where project=? and service_name=?), " +
        "?, ?, ?, ?, ?, ?)", [project, host_data.name,
      project, host_data.name, host_data.health, host_data.ip,
