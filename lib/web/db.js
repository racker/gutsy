...
module.exports = new utils.make_class({
  ...
  add_event: function(event, project, cb){
    ...
    //console.log(details);
    self.db.run("INSERT INTO events(project, username, time, points, related_api, details, story_points, multiplier) values (?, ?, ?, ?, ?, ?, ?, ?);",
      [project, event.user, event.date, event.get_points(), event.related_api, details, event.story_points, event.multiplier],
      cb);
  },
