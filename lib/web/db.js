...
module.exports = new utils.make_class({
  ...
  get_projects: function (name, cb) {
    ...
    if (project === undefined) {
      ...
      return self.db.get("SELECT * FROM projects WHERE name = ?;", [project], cb);
    }
  },
  add_project: function (name, url, devops_json, creds, cb) {
    var self = this;
    return self.db.run("INSERT INTO projects (name, url, devops_json, creds) VALUES (?, ?, ?, ?);", [name, url, data, creds], self.squelch_integrity_error(cb));
  },
  update_project: function (id, name, url, devops_json, cb) {
    var self = this;
    // TODO: update creds
    return self.db.run("UPDATE projects SET name = ?, url = ?, devops_json = ?, updated_at = ? WHERE id = ?;", [name, url, devops_json, Date.now(), id], self.squelch_integrity_error(cb));
  }
})();
