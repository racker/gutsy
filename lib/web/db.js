...
module.exports = new utils.make_class({
  ...
  get_highscores: function(project, start, end, cb){
    var self = this;
    var query = "SELECT COALESCE(display_name, username) as name, username, sum(points) as score from events " +
  ...
  get_highscore_breakdown: function(project, start, end, username, cb){
    var self = this;
    var query = "SELECT COALESCE(display_name, username) as name, username, time, points, related_api, " +
  ...
  get_aliases: function(display_name, cb){
    var self = this;
    self.db.get("SELECT * FROM aliases WHERE alias=?;", display_name, cb);
  },
