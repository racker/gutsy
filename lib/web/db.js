...
module.exports = new utils.make_class({
  ...
  get_highscore_breakdown: function(project, start, end, username, cb){
    var self = this;
    var query = "SELECT COALESCE(display_name, username), time, points, related_api, story_points, multiplier, " +
    "url, title, id from events outer left join aliases on username=alias ";
    var args = [];
    var where = "";
    if (start){
      where += " time>=? AND ";
