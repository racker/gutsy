...
"CREATE TABLE IF NOT EXISTS version_one_poll (project varchar(40), time timestamp default CURRENT_TIMESTAMP, "+
...
stats = utils.make_class({
  ...
  _query: function(query, values, cb, exit_on_error){
    ...
    self.db.run(query, values, function(err, results){
      if (err){
        console.error(err);
        if(exit_on_error){
  ...
  set_previous_v1_poll_date: function(project, date, cb){
    var self = this;
    self._query("INSERT INTO version_one_poll(project, time) values(?, ?)",
      [project, date],
      cb);
  },
  get_previous_v1_poll_date: function(project, cb){
    var self = this;
    self._query("SELECT max(time) as date FROM version_one_poll where project=?",
      [project],
      cb);
  },
  add_event: function(event, project, cb){
    ...
    self._query("INSERT INTO events(project, username, time, points, related_api) values (?, ?, ?, ?, ?)",
