...
module.exports = new utils.make_class({
  ...
  add_event: function(event, project, cb){
    ...
    self.db.run("INSERT INTO events(project, username, time, points, related_api, details) values (?, ?, ?, ?, ?, ?);",
      [project, event.user, event.date, event.get_points(), event.related_api, details],
      cb);
  },
