...
var SCHEMA = ["CREATE TABLE IF NOT EXISTS events (project VARCHAR(40) NOT NULL, " +
  ...
  "CREATE TABLE IF NOT EXISTS devops (id INTEGER PRIMARY KEY NOT NULL, "+
  "project VARCHAR(60) NOT NULL, " +
  "url VARCHAR(1024) NOT NULL, " +
  "data TEXT, " +
  "updated_at TIMESTAMP, " +
  "UNIQUE(project), CHECK(project <> ''));",
...
module.exports = new utils.make_class({
  ...
  get_devops: function (project, cb) {
    var self = this;
    if (project === undefined) {
      return self.db.all("SELECT * FROM devops;", cb);
    }
    else {
      return self.db.get("SELECT * FROM devops WHERE project = ?;", [project], cb);
    }
  },
  add_devops: function (project, url, data, cb) {
    var self = this;
    return self.db.run("INSERT INTO devops(project, url, data) VALUES (?, ?, ?);", [project, url, data], self.squelch_integrity_error(cb));
  },
  update_devops: function (id, project, url, data, cb) {
    var self = this;
    return self.db.run("UPDATE devops SET (project = ?, url = ?, data = ?, updated_at = ?) WHERE id = ?;", [project, url, data, Date.now(), id], self.squelch_integrity_error(cb));
  }
})();
