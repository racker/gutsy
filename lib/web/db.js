...
module.exports = new utils.make_class({
  ...
  add_event: function(event, project, cb){
    ...
    self.db.run("INSERT INTO events(project, username, time, points, related_api, details, story_points, event, url, title) values (?, ?, ?, ?, ?, ?, ?, ?, ? , ?);",
      [project, event.user, event.date, event.get_points(), event.related_api, event.details, event.story_points, event.event, event.url, event.title],
      cb);
  },
  ...
  get_highscore_breakdown: function(project, start, end, username, cb){
    ...
    "story_points, event, url, title, id from events outer left join aliases on username=alias " +
  ...
  get_pr_close_user: function(id, api, cb){
    var self = this;
    return self.db.get('SELECT username FROM events WHERE details=? AND related_api=? AND event=?;',
        id, api, EVENTS.closed, cb);
  }
})();
