...
var SCHEMA = ["CREATE TABLE IF NOT EXISTS events (project VARCHAR(40) NOT NULL, " +
  ...
  "CREATE TABLE IF NOT EXISTS devops_urls (id INTEGER PRIMARY KEY NOT NULL, "+
  "project VARCHAR(60) NOT NULL, " +
  "url VARCHAR(1024) NOT NULL, UNIQUE(project));",
...
module.exports = new utils.make_class({
  ...
  get_devops_url: function (project, cb) {
    var self = this;
    return self.db.get("SELECT * FROM devops_urls WHERE project = ?;", [project], cb);
  },
  get_devops_urls: function (cb) {
    var self = this;
    return self.db.all("SELECT * FROM devops_urls;", cb);
  },
  add_devops_url: function (project, url, cb) {
    var self = this;
    return self.db.run("INSERT INTO devops_urls(project, url) VALUES (?, ?);", [project, url], self.squelch_integrity_error(cb));
  },
  update_devops_url: function (id, project, url, cb) {
    var self = this;
    return self.db.run("INSERT OR REPLACE INTO devops_urls(id, project, url) VALUES (?, ?, ?);", [id, project, url], self.squelch_integrity_error(cb));
  }
})();
