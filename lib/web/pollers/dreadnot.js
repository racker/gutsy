...
module.exports = function dreadnot(payload, cb) {
    var api_config = payload.get_config();
    var requests = [];
    var factory;

    factory = new dreadnot_request_factory(api_config);
    ...
    async.parallel(requests, function(err, results){
        ...
        try{
            _.each(results, function(value){
                if (value.res.statusCode !== 200){
                    payload.set_data(value.data, cb);
                    return;
                }
        ...
        payload.set_data(null, response, cb);
    });
};
