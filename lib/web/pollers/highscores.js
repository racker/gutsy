...
function init(payload){
  db.get_last_v1_time(payload.get_config().project_name, function(err, date){
    if (err) {
      utils.die(err);
    }
...
var Poller = utils.make_class({
  ...
  _request: function (cb) {
    var self = this;
    var options, path;
    var scopes = utils.v1_scope_maker(self.api_config.project);
