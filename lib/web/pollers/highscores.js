...
var Poller = utils.make_class({
  init: function (start_time, poller) {
    var self = this;
    self.api_config = poller.get_config('version_one');
    self.get_project_name = poller.get_project_name;
  ...
  _store: function (events, callback){
    ...
    db.add_multiple_events(events, self.get_project_name());
    callback();
  }
});

module.exports = function (poller){
  db.get_last_v1_time(poller.get_project_name(), function(err, date){
    if (err) {
      utils.die("Error getting last V1 time:", err);
    }
    return new Poller(date, poller);
  });
};

module.exports.poll_interval= 15 * 60 * 1000;
