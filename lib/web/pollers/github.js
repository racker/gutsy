 ...
 * @returns
 */
var make_github_options = function(api_spec, repo, is_closed) {
  var path = [
    '/pulls'
  ];
  ...
  return utils.github_options(api_spec, repo, path);
};
...
var parse_github_response = function(data){
  ...
  avg_merge_age_metric = new MergeMetric("Merged Pull Request Average Age", "Days");
  avg_staleness_metric = new StaleMetric("Stale Pull Request Average Age", "Days");
  number_metric = new NumberMetric("Pull Requests", "Count");
 ...
 */
module.exports = function github(payload, cb) {
  var api_config = payload.get_config();
  var pulls;
  var actions = [];

  _.each(api_config.repo, function (repo) {
    var open_options = make_github_options(api_config, repo);
    var closed_options = make_github_options(api_config, repo, true);
  ...
  async.parallel(actions,
    ...
    try{
      ...
      payload.set_data(e, cb);
      return;
    }
    payload.set_data(null, data, cb);
  });
};
