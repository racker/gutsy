  ...
  request_assets: function(sprints, cb){
    ...
    _.each(self.api_config.project, function(scope) {
      _.each(sprints, function(sprint){
        defect_requests[scope + " ▼" + sprint.id] = self._sprint_asset_request_maker(sprint, scope, 'Defect');
        task_requests[scope + " ▼" + sprint.id] = self._sprint_asset_request_maker(sprint, scope, 'Task');
      });
    });
