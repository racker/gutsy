...
function bin_asset(asset, bin, sprint_start_day, sprint_window){
...
module.exports = utils.make_class({
  ...
  _sprint_asset_request_maker: function(sprint, type){
    var self = this;
    var selection = ["DetailEstimate", "ChangeDate", "AssetState", "Status.Name",
      "Timebox.Name", "Timebox", "CreateDate"];
    return function(cb){
      var path = util.format("/Hist/%s?where=Timebox='%s'&sel=%s", type, sprint.id, selection.join(','));
      var options = self.get_v1_options(path);
  ...
  parse_histories: function(history) {
    ...
    _.each(history, function (asset){
      var id = asset.id.split(":")[1];
      if (assets[id]) {
        if () {
          assets[id] = asset;
        }
  ...
  parse_assets: function(asset_classes, cb){
    ...
    _.each(asset_classes, function(assets, asset_type){
      ...
      _.each(assets, function(asset_list, sprint_id){
        ...
        bin = self.parse_histories(asset_list);
        // assets in sprint
        _.each(asset_list, function(asset) {
          bin_asset(asset, bin, sprint_start_day, self.sprint_window);
