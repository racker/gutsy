...
module.exports = utils.make_class({
  ...
  get_sprint_options: function (cb){
    ...
    if (typeof self.api_config.project === "object") {
      scope = self.api_config.project[0]; // We need any of the appropriate scopes.
    } else {
      scope = self.api_config.project; // We need any of the appropriate scopes.
    }
  ...
  parse_assets: function(asset_classes, cb){
    ...
    _.each(asset_classes, function(assets, asset_type){
      ...
      _.each(assets, function(asset_list, sprint_id){
        ...
        asset_binner = asset_binner_factory(bin, sprint_start_day, self.sprint_window);
        // assets in sprint
        _.each(asset_list, asset_binner);
      });
    });
    cb(null, bins);
  }
