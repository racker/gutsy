...
module.exports = utils.make_class({
  init: function(api_config, start, end){
    var self = this;

    var __worker;

    var v1_config = api_config.version_one ? api_config.version_one.get_config() : undefined;
    var github_config = api_config.github.get_config();
    var dreadnot_config = api_config.dreadnot ? api_config.dreadnot.get_config() : undefined;
    ...
    self.dreadnot_config = dreadnot_config;
    ...
    if(dreadnot_config){
      self.github_commit_task = self._make_last_merge_task(github_config, api_config.dreadnot.get_data());
    }
  ...
  work: function(cb){
    ...
    // XXX TODO: Giant hack
    if (!_.isArray(self.github_config.repo)) {
      self.github_config.repo = [ self.github_config.repo ];
    }
