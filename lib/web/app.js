...
exports.run = function(argv) {
  ...
  _.each(apps, function(_app){
    ...
    _app.use('/static', express.static(path.join(__dirname, '..', '..', 'extern')));
    _app.use('/static', express.static(path.join(__dirname, '..', '..', 'static')));
  ...
  pollers.install(devops, function(polling_data){
    ...
    if (secure_app){
      secure_app.use(middleware.injector.inject(polling_data, devops));
      routes.install(secure_app, status_api, devops, polling_data);
      routes_secure.install(secure_app, status_api);
