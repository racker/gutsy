...
exports.run = function(argv) {
  ...
  redirect_app.all("*", function (req, res, next) {
    return res.redirect('location', "https://" + req.headers.host + req.url, 301);
  });
  ...
  db.create_tables(function (){
    async.series({
      ...
      github_data: function (cb) {
        hooks.install(devops, function (err, results) {
          if (err) {
            console.error(err);
          }
          cb(null, results);
    ...
    function (err, results) {
      ...
      routes.web.install(web_app, devops, polling_data);
