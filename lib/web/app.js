...
exports.run = function(argv) {
  ...
  hooks = express.createServer();
  hooks.use(middleware.logger());
  ...
  db.create_tables(function (){
    ...
    function (err, results) {
      ...
      routes.web.install(web_app, status_api, devops, polling_data, idjson);
      web_app.listen(443, host);

      routes.hooks.install(hooks, devops);
      hooks.listen(port, host);
    });
  });
};
