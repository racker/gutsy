...
exports.run = function(argv) {
  ...
  startup_actions = [
    ...
    function (results, cb) {
      log.log("Loading devops...");
      load_fixtures(devops, cb);
    },
    function (devops, cb) {
      // polling_data is changed in the SIGHUP handler below
      log.log("Installing pollers...");
      pollers.install(devops, cb);
    },
    function (devops, cb) {
      log.log("Installing Github hooks...");
      hooks.install(devops, cb);
    }
  ];

  async.waterfall(startup_actions, function (err, devops) {
    if (err) {
      utils.die(err);
    }

    web_app.use(middleware.injector.inject(devops.polling_data, devops));
    routes.web.install(web_app, devops, devops.polling_data);
    ...
    process.on('SIGHUP', function() {
      ...
      load_fixtures(devops);
      async.waterfall(startup_actions, function (err, results) {
        if (err) {
          utils.die(err);
        }
      });
