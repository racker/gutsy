...
var Project = utils.make_class({
  init: function (project) {
    ...
    self.clean_devops = make_defaults({});
    self.updated_at = null;
    self.data = {};
    _.each(plugins.PLUGINS, function (polling_api, name) {
      self.data[name] = null;
    });
  ...
  start: function(cb){
    ...
    async.series([
      ...
      _.bind(self.install_plugins, self),
  ...
  load_devops: function(cb){
    ...
    self.clean_devops = self.devops_json ? JSON.parse(self.devops_json): {};
    self.clean_devops = make_defaults(self.clean_devops);
  ...
  _make_pollers: function(){
    ...
    _.each(plugins.PLUGINS, function(plugin, poller_name){
      var poller_config = {};
      var poller;
      self.pollers[poller_name] = null;

      // build configs for the poller
      _.each(plugin.related_apis, function (api_name) {
      ...
      if (plugin.related_apis.length > 0 && _.keys(poller_config).length <= 0) {
        return;
      }

      poller = new Poller(poller_name, plugin.worker, poller_config, self);
      ...
      pollers[plugin.priority] = pollers[plugin.priority] || [];

      pollers[plugin.priority].push(function(cb){
        poller.poll(cb);
      });
    });
    return pollers;
  },
  install_plugins: function(cb){
    ...
    _.each(plugins.PRIORITIES, function(priority){
      if (pollers[priority] === undefined){
        return;
      }
      plugin_series.push(function(cb){
    ...
    call_pollers = function(cb) {
      async.series(plugin_series, function(err, results){
        if (err){
          log.error(err);
        }
        if (cb) {
    ...
    call_pollers();
    self.plugin_interval_id = setInterval(call_pollers, plugins.INTERVAL);
    cb();
  },
  ...
  _uninstall_plugins: function(){
    var self = this;
    if (self.plugin_interval_id !== null) {
      clearInterval(self.plugin_interval_id);
      self.plugin_interval_id = null;
    }
  },
