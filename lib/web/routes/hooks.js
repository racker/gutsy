...
exports.install = function(app, projects){
  ...
  var v1_request = function(project, title, cb){
    ...
    var v1_info = v1.match(title);
    if (!v1_info){
      return cb(null, null);
    }

    v1_spec = project.get_api_config('version_one');
  ...
  var github_commits_request = function(project, pull_request_url){
    var github_spec = project.get_api_config('github');
    var options;
  ...
  app.post(urls.GITHUB_PUSH_API, [express.bodyParser(), middleware.project_checker(projects)], function(req, res){
    var state;
    var number_matches;
    var str_number;
    var title;
    ...
    async.parallel(requests, function(err, results){
      ...
      if (results.pr_close_user) {
        ...
        return db.add_event(event, project.name);
      }
      ...
      return db.add_multiple_events(events_list, project.name);
    });
  });
};