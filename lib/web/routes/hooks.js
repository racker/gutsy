...
exports.install = function(app, devops){
  ...
  app.post(urls.GITHUB_PUSH_API, [express.bodyParser()], function(req, res){
    ...
    console.log("new request from github", action, title, pr.user.login, e.sender.login,
    ...
    if (event_type !== null) {
      event = events.Git_Event(pr.user.login, event_type, story_points, pr.updated_at, pr.number);
