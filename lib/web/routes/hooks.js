...
exports.install = function(app, devops){
  ...
  app.post(urls.GITHUB_PUSH_API, [express.bodyParser()], function(req, res){
    ...
    if (action === "reopened"){
      ...
      db.get_db().get('SELECT username FROM events WHERE details=? AND related_api=?;', [pr.id, RELATED_APIS.github], function (err, results) {
        if (err) {
          return;
        }
        if (results) {
    ...
    merged_by_user = pr.merged_by ? pr.merged_by.login : null;
