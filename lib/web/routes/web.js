  ...
  app.get(
    ...
    function(req, res) {
      ...
      _.each(context.projects, function (project) {
        ...
        try {
          crawl_stat = fs.lstatSync(settings.saved_crawls_path + '/' + project);
          context.last_crawl_time[project] = crawl_stat.mtime;
        }
        catch (e) {
          console.log(e);
          context.last_crawl_time[project] = new Date(0);
        }
      });
    ...
    function (req, res) {
      ...
      async.parallel(options, function(err, results){
        ...
        _.each(results.aliases, function(alias){
          aliases[alias.related_api] = alias;
  ...
  app.post(
    ...
    function(req, res){
      ...
      if (data.alias.length <= 0){
        db.delete_alias(data.alias, function(err, results){
          console.error(err);
          return res.send("", 302);
        });
      }
      ...
      db.add_alias(data.alias, data.display_name, data.related_api, function(err, results){
        console.error(err);
        return res.send("", 302);
      });
