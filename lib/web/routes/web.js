...
module.exports.install = function(web_app, projects){
  web_app.use(express.errorHandler({ showStack: true, dumpExceptions: true }));

  var default_middleware = [middleware.project_checker(projects), middleware.injector.inject];
  ...
  add_route('get',
    ...
    function(req, res) {
      ...
      var respond = function(err, results){
        ...
        context.notes = notes;
        context.last_crawl_time = results.last_poll_end;
  ...
  add_route('post',
    ...
    function(req, res) {
      ...
      if (data.action === "Delete") {
        return init.delete_project(id, respond);
      }
      ...
      return init.update_project(id, data.name, data.url, data.devops_json, creds, respond);
    });
