  ...
  add_route('get',
    ...
    function(req, res) {
      var url_to_name = {},
        ...
        context,
        graph = [],
        query_parse = url.parse(req.url).query;
      ...
      _.each(req.projects, function(project){
        ...
        url_to_name[project.url] = project.name;

        if (!project.devops) {
          return;
        }

        if (!query || _.intersection(query, project.devops.tags) === query.length ){
          projects.push(project);
          _.each(project.devops.tags, function(tag){
            tags[tag] = tags[tag] ? ++tags[tag] : 1;
          });
        }

        edges[project.name] = {name: project.name, children: project.devops.dependent_services};
      ...
      var iter = function(node){
        ...
        if (!node.children || node.children.length <= 0 ){
          node.children = edges[node.name] ? edges[node.name].children : [];
        }
        console.log(node);
        _.each(node.children, function(child){
          ...
          child.name = url_to_name[child.name] || path.basename(child.name, '.json');
          children.push(iter(child));
        });
      ...
      _.each(edges, function(node, name){
        var _ = iter(node);
