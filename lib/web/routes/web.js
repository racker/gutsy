...
module.exports.install = function(web_app, installed_projects){
  ...
  add_route('get',
    urls.META_INDEX,
    function(req, res) {
      var links = [], url_to_id = {}, edges = [], vertexes = [], vertex;
      var query_parse = url.parse(req.url).query;
      var query;
      var meta_index_projects = {};
      var tags = [];
      var query_tags = [];
      var intersection;
      var tag_count = {};
      ...
      var make_meta_index_project = function(project) {
        var ruok = project.get_data("ruok") || {};
        var devops = project.devops || {};

        meta_index_projects[project.id] = {
          formatted_project_name: project.name.replace(/\ /g,'\-'),
          clean_devops: project.clean_devops,
          tags: devops.tags ? devops.tags.join(', ').toLowerCase() : "",
      ...
      _.each(req.projects, function(project){
        vertexes.push(project.id);
        if (!query){
          make_meta_index_project(project);
        } else{
        ...
        url_to_id[project.url] = project.id;
        ...
        _.each(project.devops.dependent_services, function(dep){
          links.push({id: project.id, url: dep});
        });
      });
      ...
      _.each(links, function(link){
        vertex = url_to_id[link.url];
        if (!vertex){
          return;
        }
        edges.push([link.id, vertex]);
        dependency_count[vertex] = (dependency_count[vertex] === undefined ? 1 : dependency_count[vertex] + 1 );
        dependency_count[link.id] = (dependency_count[link.id] === undefined ? -1 : dependency_count[link.id] - 1 );
        linked_vertexes.push(link.id);
        linked_vertexes.push(vertex);
      });

      vertexes.sort(function(a,b){
        var count_a = dependency_count[a] !== undefined ? dependency_count[a] : -1000000;
        var count_b = dependency_count[b] !== undefined ? dependency_count[b] : -1000000;
      ...
      context = {
        ...
        edges: edges,
        vertexes: vertexes,
        linked_vertexes: _.uniq(linked_vertexes)
      };
