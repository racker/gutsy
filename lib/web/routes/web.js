...
module.exports.install = function(app, devops, polling_data, idjson){
    ...
    function(req, res){
      ...
      db.get_highscores(project, start_date, end_date, function(errors, results){
        ...
        for (i = 0; i < correlated_data.length; i++){
          ...
          row.place = place;
          row.href = req.path + "/" + encodeURI(row.username) + "?" + query_string;
        }
  ...
  app.get(
    ...
    function (req, res) {
      ...
      db.get_highscore_breakdown(project, start_date, end_date, username, function (errors, results) {
        var context = {
          username: username,
          events: results
        };
