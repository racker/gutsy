...
module.exports.install = function(web_app){
  ...
  add_route('post',
    ...
    function(req, res) {
      ...
      if (data.id) {
        if (data.method === "delete") { // lame workaround because forms can only POST and GET
          actions.unshift(function (cb) {
            db.delete_project(data.id, cb);
          });
        }
        else {
          actions.unshift(function (cb) {
            db.update_project(data.id, data.name, data.url, data.devops_json, data.creds, cb);
          });
        }
      } else {
        actions.unshift(function (cb) {
          db.add_project(data.name, data.url, data.devops_json, data.creds, cb);
        });
      }
  ...
  add_route('post',
    ...
    function (req, res) {
      crawler.run(function (err, results) {
        ...
        return res.redirect(url.META_INDEX);
      });
    });
