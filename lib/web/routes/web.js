  ...
  app.get(
    ...
    function(req, res) {
      ...
      _.each(context.projects, function (project) {
        ...
        try {
          crawl_stat = fs.lstatSync(settings.saved_crawls_path + '/' + project.name.toLowerCase());
          context.last_crawl_time[project.name.toLowerCase()] = crawl_stat.mtime;
        }
        catch (e) {
          console.log(e);
          context.last_crawl_time[project.name.toLowerCase()] = new Date(0);
        }
      });
