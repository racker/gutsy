...
module.exports.install = function(web_app){
  ...
  add_route('get',
    ...
    function(req, res) {
      ...
      var respond = function(err, results){
        ...
        var jsonify = false;
        var rssify = false;
        var related_api = get_related_api(req, 'version_one');
        if (err){
          context.errors = err;
        }
        if (results){
        ...
        context.notes = notes;
        context.last_crawl_time = release_notes_poller.get_last_crawl_time();
      ...
      project = get_project(req);
      release_notes_poller = project.pollers.release_notes;
      poller_data = release_notes_poller.get_data();

      if(!start && !end){
        if (poller_data) {
          context.end = poller_data.end;
          context.start = poller_data.start;
        }
        else {
          context.end = moment().subtract('minutes', 5);
          context.start = context.end.clone().subtract('days', 7);
        }
        return res.render('release_notes.jade', context);
      }
      ...
      worker = new ReleaseNotesWorker(start, end, release_notes_poller).work(respond);
  });
  ...
  add_route('post',
    ...
    function(req, res) {
      ...
      var respond = function(err, project){
        ...
        if (project && project.devops && project.devops.needs_creds){
      ...
      if (data.id) {
        ...
        project = PROJECTS[data.old_name];
        if (_.isEmpty(creds)){
          creds = undefined;
        } else{
          creds = _.isEmpty(project.creds) ? creds : _.defaults(creds, project.creds);
          creds = JSON.stringify(creds);
        }
        return init.update_project(data.id, data.name, data.url, data.devops_json, creds, respond);
      }
  ...
  add_route('post',
    urls.CRAWL,
    function (req, res) {
      init.install(function (err, result) {
