...
var log = require('../../log');
var ReleaseNotesWorker = require('../release_notes_worker');
...
module.exports.install = function(app, devops, polling_data){
  ...
  app.get(
    ...
    function(req, res) {
      ...
      worker = new ReleaseNotesWorker(polling_data[req.params.project], start, end);
  ...
  app.get(
    ...
    function(req, res) {
      ...
      _.each(context.projects, function (project) {
        ...
        try {
          crawl_stat = fs.lstatSync(settings.saved_crawls_path + '/' + project.project);
          context.last_crawl_time[project.project] = crawl_stat.mtime;
        }
        catch (e) {
          log.error(e);
          context.last_crawl_time[project.project] = new Date(0);
        }
      });

      db.get_devops(undefined, function (err, results) {
        _.each(results, function (value, key) {
          if (devops[value.project] === undefined) {
            devops[value.project] = {};
          }
          devops[value.project].url = value.url;
        });
  ...
  app.post(
    ...
    function(req, res) {
      var data = req.body;
      var redirect = function (err, results) {
        return res.redirect(urls.META_INDEX);
      };

      if (data.id) {
        db.update_devops(data.id, data.project, data.url, undefined, redirect);
      }
      else {
        db.add_devops(data.project, data.url, undefined, redirect);
      }
    });
