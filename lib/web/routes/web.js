...
module.exports.install = function(web_app){
  ...
  add_route('get',
    ...
    function(req, res) {
      ...
      var respond = function(err, results){
        ...
        if (req.query.format && req.query.format.toLowerCase() === 'json'){
          for (i=0; i<context.notes.length; i++){
            notes.push(context.notes[i].toJSON(get_related_api(req, 'version_one')));
          }
      ...
      if(!start && !end){
        context.end = poller_data ? poller_data.end : new Date();
        context.start = poller_data ? poller_data.start : new Date(context.end - 7 * 24 * 60 *60 * 1000);
        return res.render('release_notes.jade', context);
      }
