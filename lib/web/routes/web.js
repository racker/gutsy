  ...
  add_route('get',
    ...
    function(req, res) {
      ...
      _.each(req.projects, function(project){
        ...
        intersection = _.intersection(query, project.devops.tags);
      ...
      _.each(tags, function(tag) {
        tag_count[tag] = tag_count[tag] !== undefined ? ++tag_count[tag] : 1;
      });

      sorted_tags = _.keys(tag_count);
      // sort by count then alphabetically- this doesn't quite work, but is close enough
      // really, we'd need to bin by count then do a sort
      sorted_tags.sort(function(a, b){
        var dif = tag_count[b] - tag_count[a];
        return (dif !== 0 ? dif : b < a);
      });
