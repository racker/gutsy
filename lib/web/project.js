...
var Project = utils.make_class({
  ...
  update: function(id, name, url, devops_json, creds_json, cb) {
    ...
    if (name !== undefined && self.name !== name) {
      ...
      update = true;
    } else if (self.creds !== undefined && !_.isEqual(JSON.parse(creds_json), self.creds)) {
      update = true;
    } else if (self.devops_json !== undefined && !_.isEqual(JSON.parse(devops_json), JSON.parse(self.devops_json))) {
      update = true;
    }
    ...
    db.update_project(id, name, url, devops_json, creds, function (err) {
      if (name !== undefined && self.name !== name) {
        PROJECTS[name] = self;
        self.name = name;
