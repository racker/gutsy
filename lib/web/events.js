...
var __Event = utils.make_class({
  init: function(user, status, related_api, story_points, date, url, title, details){
    var self = this;
...
exports.Git_Event = function(user, status, story_points, pr){
  var date = pr.updated_at;
  return new __Event(user, status, RELATED_APIS.github, story_points, date, pr.html_url, pr.title, pr.id);
};
...
exports.V1_Event = function(api_config, v1_asset){
  ...
  if (v1_status === '64'){
    ...
    status = exports.EVENTS.v1_other;
  }
  title = v1_asset.id;
  url = utils.v1_id_to_url(api_config, v1_asset.id);

  return new __Event(user, status, RELATED_APIS.version_one,
    100, create_date, url, title, v1_asset.id);
};
