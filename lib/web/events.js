...
exports.V1_Event = function(v1_asset, story_points){
  var create_date = new Date(asset.attributes.CreateDate).valueOf();
  var user = asset.attributes['ChangedBy.Name'];
  var v1_status = asset.attributes.AssetState;
  var status;
  if (v1_status === '64'){
    status = exports.v1_active;
  } else if( v1_status === '128'){
    status = exports.v1_closed;
  } else{
    status = exports.none;
  }
  return new __Event(user, null, status, RELATED_APIS.version_one,
    story_points, create_date, asset);
};
