...
module.exports.install = function(app, status_api, devops, polling_data){
  app.get(
    '/favicon.ico',
    function(req, res) {
      res.send("favicon");
  ...
  app.post(
    ...
    function(req, res) {
      ...
      worker = new Release_Notes_Worker(polling_data[req.params.project], context.start, context.end);
  ...
  app.get(
    ...
    function(req, res){
      ...
      db.get_stats(project, start_date, end_date, function(errors, results){
        if (results){
        ...
        res.render('highscores.jade', {data: results, errors: errors});
      });
    }
  );
};
