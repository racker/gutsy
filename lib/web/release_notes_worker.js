...
module.exports = utils.make_class({
  init: function(polling_data, start, end){
    ...
    var v1_config = polling_data.version_one.get_config();
    var github_config = polling_data.github.get_config();
    var dreadnot_config = polling_data.dreadnot.get_config();
  ...
  _make_last_merge_task: function(github_config, dread_data){
    ...
    return {
      ...
      cb: _.bind(self.github_commit_task_cb, self)
    };
  },
  ...
  __v1_response_handler: function(note, errors, results){
    ...
    utils.parse_v1(results.data, function(error, asset){
      ...
      asset = asset[0];
      note.set_data(asset.attributes.Description.text);
      note.set_create_date(asset.attributes.CreateDate.text);
