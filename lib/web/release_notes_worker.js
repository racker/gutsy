...
var Note = utils.make_class({
  init: function(pull_request){
    ...
    self.master_sha = pull_request.head.sha;
    self.v1 = {
      data: null,
      href: null,
      error: null,
...
module.exports = utils.make_class({
  init: function(polling_data, start, end){
    ...
    var sha, dreadnot_data, github_commit_task_cb;
    ...
    if (dreadnot_config){
      dreadnot_data = polling_data.dreadnot.data;
      if (!dreadnot_data){
        return;
      }
      sha = dreadnot_data[0].deploy.to_revision;
      github_commit_task_cb = function(err, results){
        ...
        self.date_of_last_merge_before_deploy = new Date(data.commit.committer.date);
      };
      self.github_commit_task = {
        options: {
  ...
  work: function(cb){
    ...
    if (self.github_commit_task){
      self._q.push(self.github_commit_task);
    }
