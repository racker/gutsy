...
var v1_options = function(v1_config, path){
  path = path.join('');
  return {
    port: v1_config.port,
    host: v1_config.host,
    path: path + '&sel='+ V1_SELECTION,
    method: 'GET',
    headers: {
...
var get_v1_history_options = function(v1_config, start, end, type){
  var self = this;
  ...
  path = ['/',
    ...
    "/rest-1.v1/Hist/",
    type,
    util.format("?where=Scope=%s;ChangeDate>'%s';ChangeDate<'%s'",
      utils.v1_scope_maker(v1_config.project), start.toISOString(), end.toISOString())
  ];
  return v1_options(v1_config, path);
};

var get_v1_options_from_branch_name = function(v1_config, v1_story_info){
  var options, path, identifier;
  path = ['/',
    v1_config.name,
    "/rest-1.v1/Data/",
    v1_story_info.long_name,
    util.format("?where=Number='%s'", v1_story_info.number)
  ];
  return v1_options(v1_config, path);
};
...
module.exports = utils.make_class({
  ...
  _history_handler: function(type, err, results){
    var self = this;
    self.v1_data[type] = {};
    utils.parse_v1(results.data, function(err, assets){
      _.each(assets, function(asset){
        if (utils.v1_asset_is_open(asset) === true){
          return;
        }
  ...
  _get_v1_history: function(){
    ...
    _.each(['Story', 'Defect'], function(type){
      var options = get_v1_history_options(self.v1_config, self._start, self._end, type);
