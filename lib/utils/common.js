...
exports.make_class = function(proto){
  var f = function(){
    var invoked_with_new = this instanceof f;
    // did you make me with new?
    // NOTE: we could use arguments.callee, but that doesn't work with strict
    if (!invoked_with_new){
...
exports.request_maker = function(options, call_back) {
  ...
  if (options.post_data !== undefined){
    ...
    if (options.dont_stringify === undefined){
      post_data = JSON.stringify(post_data);
    }else{
      delete options.dont_stringify;
    }
  ...
  method = options.port === 443 ? https : http;
